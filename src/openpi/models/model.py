        ...
        if train:
            ...
            if "wrist" not in key:
                ...
                if max_h > 0 and max_w > 0:
                    start_h = torch.randint(0, max_h + 1, (1,)).item()
                    start_w = torch.randint(0, max_w + 1, (1,)).item()
                ...
                angle = torch.rand(1).item() * 10 - 5  # Random angle between -5 and 5 degrees
                if abs(angle) > 0.1:  # Only rotate if angle is significant
                    # Convert to radians
                    angle_rad = torch.tensor(angle * torch.pi / 180.0, device=image.device)
            ...
            # Random brightness
            brightness_factor = 0.7 + torch.rand(1).item() * 0.6  # Random factor between 0.7 and 1.3
            image = image * brightness_factor

            # Random contrast
            contrast_factor = 0.6 + torch.rand(1).item() * 0.8  # Random factor between 0.6 and 1.4
            ...
            saturation_factor = 0.5 + torch.rand(1).item() * 1.0  # Random factor between 0.5 and 1.5
